...
class AdjacencyMatrixFA:
    ...
    def accepts(self, word: Iterable[Symbol]) -> bool:
        ...
        for letter in word:
            if self.matrix.get(letter) is None:
                return False

            for s1, s2 in product(states, self.states.keys()):
                if self.matrix[letter][self.states[s1], self.states[s2]]:
                    states.add(s2)
...
def intersect_automata(
    ...
    intersection_matrix.matrix = {
        key: kron(
            automaton1.matrix[key],
            automaton2.matrix[key],
            format="csr",
        )
        for key in automaton1.matrix.keys()
        if key in automaton2.matrix
    }
